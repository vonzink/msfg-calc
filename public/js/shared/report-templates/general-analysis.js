...
(function () {
  ...
  RT.register('cash-vs-mortgage', function (doc) {
    ...
    return pdfKeyValue(data,
      ...
      'Recommendation', null
    ).concat([{ text: res.recommendation, bold: true, fontSize: 9, color: '#2d6a4f', margin: [0, 4, 0, 0] }]);
  });
  ...
  RT.register('buy-vs-rent', function (doc) {
    ...
    ).concat(res.recommendation ? [{ text: res.recommendation, bold: true, fontSize: 9, color: '#2d6a4f', margin: [0, 4, 0, 0] }] : []);
  });
  ...
  RT.register('refi', function (doc) {
    ...
    content.push({ table: { headerRows: 1, widths: ['*', 100, 100], body: loanBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });

    content.push({ text: 'Refinance Now', style: 'sectionTitle', margin: [0, 4, 0, 2] });
    var nowBody = [
    ...
    content.push({ table: { headerRows: 1, widths: ['*', 100], body: nowBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });

    if (data.costOfWaiting) {
      var cow = data.costOfWaiting;
      content.push({ text: 'Cost of Waiting', style: 'sectionTitle', margin: [0, 4, 0, 2] });
      var cowBody = [
        [{ text: 'Metric', style: 'tableHeader' }, { text: 'Value', style: 'tableHeader', alignment: 'right' }]
      ];
      ...
      content.push({ table: { headerRows: 1, widths: ['*', 100], body: cowBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });
      ...
      if (cmpBody.length > 1) content.push({ table: { headerRows: 1, widths: ['*', 90, 90], body: cmpBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });

      if (cow.difference) {
        content.push({ columns: [{ text: 'Net Difference', bold: true, fontSize: 9, color: '#2d6a4f' }, { text: cow.difference, alignment: 'right', bold: true, fontSize: 9, color: '#2d6a4f' }], margin: [0, 2, 0, 0] });
      }
    }

    if (data.doubleRefi) {
      var dr = data.doubleRefi;
      content.push({ text: 'Refi Twice Strategy', style: 'sectionTitle', margin: [0, 4, 0, 2] });
      var drBody = [
        [{ text: 'Metric', style: 'tableHeader' }, { text: 'Value', style: 'tableHeader', alignment: 'right' }],
      ...
      content.push({ table: { headerRows: 1, widths: ['*', 100], body: drBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });
      ...
      content.push({ table: { headerRows: 1, widths: ['*', 80, 80, 80], body: triBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });

      if (dr.bestStrategy) {
        content.push({ columns: [{ text: 'Best Strategy', bold: true, fontSize: 9, color: '#2d6a4f' }, { text: dr.bestStrategy, alignment: 'right', bold: true, fontSize: 9, color: '#2d6a4f' }], margin: [0, 2, 0, 0] });
      }
    }

    if (data.recommendation) {
      content.push({ canvas: [{ type: 'line', x1: 0, y1: 0, x2: 532, y2: 0, lineWidth: 1, lineColor: '#2d6a4f' }], margin: [0, 4, 0, 2] });
      content.push({ text: data.recommendation, bold: true, fontSize: 9, color: '#2d6a4f', margin: [0, 2, 0, 0] });
  ...
  RT.register('amortization', function (doc) {
    ...
    if (data.yearlySummary && data.yearlySummary.length) {
      content.push({ text: 'Annual Breakdown', style: 'sectionTitle', margin: [0, 4, 0, 2] });
      var yrBody = [[
      ...
      data.yearlySummary.forEach(function (yr) {
        yrBody.push([{ text: yr.year, fontSize: 7 },
          { text: fmt(yr.principal), fontSize: 7, alignment: 'right' },
          { text: fmt(yr.interest), fontSize: 7, alignment: 'right' },
          { text: yr.extra ? fmt(yr.extra) : '—', fontSize: 7, alignment: 'right' },
          { text: fmt(yr.payments), fontSize: 7, alignment: 'right' }
        ]);
      });
      content.push({ table: { headerRows: 1, widths: ['*', 65, 65, 50, 65], body: yrBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 4] });
    }

    if (data.schedule && data.schedule.length) {
      content.push({ text: 'Full Amortization Schedule', style: 'sectionTitle', margin: [0, 4, 0, 2], pageBreak: 'before' });
      var sBody = [[
      ...
      data.schedule.forEach(function (pmt) {
        sBody.push([
          { text: String(pmt.num), fontSize: 6.5 }, { text: pmt.date, fontSize: 6.5 },
          { text: fmt(pmt.payment), fontSize: 6.5, alignment: 'right' },
          { text: fmt(pmt.principal), fontSize: 6.5, alignment: 'right' },
          { text: fmt(pmt.interest), fontSize: 6.5, alignment: 'right' },
          { text: pmt.extra ? fmt(pmt.extra) : '—', fontSize: 6.5, alignment: 'right' },
          { text: fmt(pmt.balance), fontSize: 6.5, alignment: 'right' }
        ]);
      });
      content.push({ table: { headerRows: 1, widths: [18, 48, 58, 58, 58, 42, 68], body: sBody }, layout: 'lightHorizontalLines' });
    }
    return content;
  });
  ...
  RT.register('budget',
    ...
    function (doc) {
      ...
      return {
        ...
        prepDate: txt(doc, 'bgPrepDate'),
        loanPurpose: txt(doc, 'bgLoanPurpose'),
        propertyValue: val(doc, 'bgPropertyValue'),
        loanAmount: val(doc, 'bgLoanAmount'),
        rate: val(doc, 'bgRate'),
        ...
        housing: {
          pi: val(doc, 'bgPI'), tax: val(doc, 'bgPropertyTax'),
          insurance: val(doc, 'bgHomeInsurance'), mi: val(doc, 'bgMI'),
        ...
        living: {
          ...
          insurance: val(doc, 'bgInsurance'), childcare: val(doc, 'bgChildcare'),
          entertainment: val(doc, 'bgEntertainment'), otherLiving: val(doc, 'bgOtherLiving')
        },
        reserves: {
          checking: val(doc, 'bgChecking'), savings: val(doc, 'bgSavings'),
          retirement: val(doc, 'bgRetirement'), investments: val(doc, 'bgInvestments'),
          otherAssets: val(doc, 'bgOtherAssets')
        },
        totals: {
          qualIncome: txt(doc, 'bgTotalQualIncome'),
          totalIncome: txt(doc, 'bgGrandTotalIncome'),
          housing: txt(doc, 'bgHousingTotal'),
          liabilities: txt(doc, 'bgLiabilitiesTotal'),
          living: txt(doc, 'bgLivingTotal'),
          expenses: txt(doc, 'bgGrandTotalExpenses'),
          frontDTI: txt(doc, 'bgFrontDTI'),
          backDTI: txt(doc, 'bgBackDTI'),
    ...
    function (data) {
      ...
      if (data.loanAmount) html += '<div class="rpt-param"><span>Loan Amount</span><span>' + fmt0(data.loanAmount) + '</span></div>';
      if (data.rate) html += '<div class="rpt-param"><span>Interest Rate</span><span>' + ratePct(data.rate) + '</span></div>';
      if (data.termMonths) html += '<div class="rpt-param"><span>Term</span><span>' + data.termMonths + ' months</span></div>';
      ...
      /* Housing */
      html += '<div class="rpt-section"><h4 class="rpt-section-title">Proposed Monthly Housing Payment</h4>';
      html += '<table class="rpt-table"><thead><tr><th>Item</th><th class="rpt-num">Monthly</th></tr></thead><tbody>';
      html += incRow('Principal & Interest', hou.pi);
      html += incRow('Property Taxes', hou.tax);
      html += incRow('Homeowners Insurance', hou.insurance);
      html += incRow('Mortgage Insurance', hou.mi);
      html += incRow('HOA / Condo Fees', hou.hoa);
      html += incRow('Flood Insurance', hou.flood);
      html += incRow('Other Housing', hou.other);
      html += '</tbody></table>';
      ...
      /* DTI & Residual */
      html += '<div class="rpt-section"><h4 class="rpt-section-title">DTI Analysis & Residual Income</h4>';
      html += '<div class="rpt-comparison"><div class="rpt-compare-col">';
      ...
      if (liv.utilities || liv.telecom || liv.groceries || liv.transport || liv.insurance || liv.childcare || liv.entertainment || liv.otherLiving) {
        html += '<div class="rpt-section"><h4 class="rpt-section-title">Living Expenses</h4>';
        html += '<table class="rpt-table"><tbody>';
        html += incRow('Utilities', liv.utilities);
        html += incRow('Phone / Internet', liv.telecom);
        html += incRow('Groceries', liv.groceries);
        html += incRow('Transportation', liv.transport);
        html += incRow('Insurance (auto/health/life)', liv.insurance);
        html += incRow('Childcare / Education', liv.childcare);
        html += incRow('Subscriptions / Entertainment', liv.entertainment);
        html += incRow('Other Living', liv.otherLiving);
        html += '</tbody></table>';
        html += '<div class="rpt-grand-total"><span>Total Living Expenses</span><span>' + tot.living + '</span></div>';
        html += '</div>';
      }

      /* Net cash flow */
      html += '<div class="rpt-section"><h4 class="rpt-section-title">Monthly Cash Flow</h4>';
      html += '<div class="rpt-grand-total"><span>Net Cash Flow (All Income - All Expenses)</span><span>' + tot.cashFlow + '</span></div>';
      html += '</div>';

      /* Reserves */
      if (res.checking || res.savings || res.retirement || res.investments || res.otherAssets) {
        html += '<div class="rpt-section"><h4 class="rpt-section-title">Savings & Reserves</h4>';
        html += '<table class="rpt-table"><tbody>';
        html += incRow('Checking', res.checking);
        html += incRow('Savings', res.savings);
        html += incRow('Retirement / 401k', res.retirement);
        html += incRow('Investments', res.investments);
        html += incRow('Other Assets', res.otherAssets);
        html += '</tbody></table>';
        html += '<div class="rpt-grand-total"><span>Total Reserves</span><span>' + tot.reserves + '</span></div>';
        html += '<div class="rpt-subtotal"><span>Months of Reserves</span><span>' + tot.reserveMonths + '</span></div>';
        html += '</div>';
      }
    ...
    function (data) {
      ...
      if (loanRows.length) {
        ...
        content.push({ table: { widths: ['*', 'auto'], body: loanRows }, layout: { hLineWidth: function() { return 0; }, vLineWidth: function() { return 0; }, paddingLeft: function() { return 3; }, paddingRight: function() { return 3; }, paddingTop: function() { return 1.5; }, paddingBottom: function() { return 1.5; } }, margin: [0, 0, 0, 4] });
      }
        ...
        columns: [
          {
            ...
            stack: [
              ...
              { table: { headerRows: 1, widths: ['*', 55, 60], body: incBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 2] },
          ...
          { width: '3%', text: '' },
          {
            ...
            stack: [
              ...
              { table: { headerRows: 1, widths: ['*', 65], body: houBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 2] },
      ...
      content.push({ table: { headerRows: 1, widths: ['*', 55, 60, 55, 42], body: liabBody }, layout: 'lightHorizontalLines', margin: [0, 0, 0, 2] });
