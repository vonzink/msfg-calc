...
(function () {
  ...
  extractors['fee-worksheet'] = function (doc) {
    // Capture custom line items from dynamically added rows
    var customItems = [];
    ...
    return {
      ...
      // Funds summary (all numeric now — inputs not spans)
      ...
      // Monthly housing (all numeric now — inputs not spans)
  ...
  renderers['fee-worksheet'] = function (data) {
    ...
    /* Render custom items for a given section */
  ...
  pdfGenerators['fee-worksheet'] = function (data) {
    ...
    /* Fee sections with custom items */
  ...
  renderers['compare'] = function (data) {
    ...
    /* helper: find best (lowest positive) index */
    ...
    function feeRow(label, key) {
      // Only show if any loan has a non-zero value
      var hasVal = false;
    ...
    /* Total interest & APR */
    html += fmtRow('Total Interest', 'totalInterest', fmt, true);

    // Compute totalInterest for each loan if not in data
    html = html.replace(/<tr><td>Total Interest<\/td>/g, '');
