...
(function() {
  ...
  function getRequiredResidual(loanAmt, region, familySize) {
    ...
    const add = loanAmt >= 80000 ? residualData.additionalOver80k : residualData.additionalUnder80k;
    const size = Math.min(familySize, 5);
    let base = residualData[table][region][size];
    if (familySize > 5) base += (familySize - 5) * add;
    return base;
  }

  function calculate() {
    const mortAmt = pn('mortgageAmount');
    const rate = document.getElementById('interestRate').value;
    const term = parseInt(document.getElementById('loanTerm').value);
    const gross = pn('grossIncome');
    const fam = parseInt(document.getElementById('familySize').value);
    const region = document.getElementById('region').value;

    const factor = getPaymentFactor(rate, term);
    const pi = (mortAmt / 1000) * factor;
    document.getElementById('paymentFactor').value = factor.toFixed(2);
    document.getElementById('piPayment').value = pi.toFixed(2);

    const taxes = pn('propertyTaxes'), ins = pn('homeInsurance'), hoa = pn('hoaDues');
    const housing = pi + taxes + ins + hoa;
    document.getElementById('totalHousing').textContent = fmt(housing, 0);

    const car = pn('carPayments'), rev = pn('revolvingAccounts'), inst = pn('installmentLoans');
    const child = pn('childCare'), other = pn('otherDebts');
    const debts = car + rev + inst + child + other;
    document.getElementById('totalDebts').textContent = fmt(debts, 0);

    const sqft = pn('squareFootage');
    const maint = sqft * 0.14;
    ...
    const fedTax = pn('federalTax'), stateTax = pn('stateTax'), ss = pn('socialSecurity');
    const totalTax = fedTax + stateTax + ss;
    document.getElementById('totalTaxes').textContent = fmt(totalTax, 0);

    const reqRes = getRequiredResidual(mortAmt, region, fam);
    const actRes = gross - housing - debts - maint - totalTax;
    ...
    const resEl = document.getElementById('residualStatus');
    if (actRes >= reqRes) {
    ...
    const dti = gross > 0 ? ((housing + debts) / gross) * 100 : 0;
    const dtiEl = document.getElementById('dtiRatio');
    ...
    renderCalcSteps(pi, housing, debts, maint, totalTax, gross, actRes, reqRes, dti, factor, mortAmt);
  }

  function renderCalcSteps(pi, housing, debts, maint, tax, gross, actRes, reqRes, dti, factor, mortAmt) {
    const c = document.getElementById('calcSteps-vaprequal');
    if (!c) return;
    let html = '';
  ...
  function buildResidualTables() {
    ...
    ['under80k','over80k'].forEach(function(key) {
      const body = document.getElementById('residual' + (key === 'under80k' ? 'Under80k' : 'Over80k') + 'Body');
      body.innerHTML = '';
      for (let s = 1; s <= 5; s++) {
        let row = '<tr><td>' + s + '</td>';
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Bind all input/select elements that trigger recalculation
    const calcInputIds = [
      'familySize', 'region', 'mortgageAmount', 'interestRate', 'loanTerm',
      'grossIncome', 'propertyTaxes', 'homeInsurance', 'hoaDues',
      'carPayments', 'revolvingAccounts', 'installmentLoans', 'childCare', 'otherDebts',
      'squareFootage', 'federalTax', 'stateTax', 'socialSecurity'
    ];
    ...
    document.getElementById('refTableToggle').addEventListener('click', toggleRefTables);
